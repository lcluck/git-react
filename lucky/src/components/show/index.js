import React, { Component } from 'react';

import "./index.css";

import axios from "axios";

import ReactDOM from 'react-dom';
import ReactSwipe from 'react-swipe';

import {connect} from "react-redux";

class Show extends Component {
  constructor(props){
    super(props);
    this.state={
        showlist:{},
        movieslist:[],
       
    }
  }
  render() {
    return (
  
      <div className="body-warp">
           
              {
                <div>
                <div className="cinema-warp">
                     <div className="cinema-info">
                          <div className="title line-ellipsis">{this.state.showlist.nm}</div>
                          <div className="title2 line-ellipsis">{this.state.showlist.addr}</div> 
                          <div className="location-icon">
                               <img className="location-img" src="data:image/png;base64," alt=""/>  
                          </div> 
                     </div>
                     <div>                                   
                     </div>
                </div>
                <div className="cinema-nav">
                <ReactSwipe className="post-bg" swipeOptions={{continuous: true,speed: 400}}>
                      <ReactSwipe className="swipe-wrap">
                           <div className="swipe-slide">
                        
                               <div className="post">
                                    <img src="https://p0.meituan.net/148.208/movie/238e2dc36beae55a71cabfc14069fe78236351.jpg" alt=""/>  
                               </div>
                                <div className="post">
                                    <img src="https://p1.meituan.net/148.208/movie/8ea36c210973ef97616be7ca1f0d5fc0216565.jpg" alt=""/>  
                               </div>
                               <div className="post">
                                    <img src="https://p0.meituan.net/148.208/movie/ee2e68492504b2390ce246f1e8e6d8ba1550779.jpg" alt=""/>  
                               </div>
                                <div className="post">
                                    <img src="https://p1.meituan.net/148.208/movie/6383f5cb72f994370f9e817eaa495aaf428644.jpg" alt=""/>  
                               </div>
         
                           </div> 

                      </ReactSwipe> 
                          
                </ReactSwipe>
                     
                </div>
                </div>
              }
           
      </div>
       
    )
  }
  componentDidMount(){
    axios.get(`/ajax/cinemaDetail?cinemaId=${localStorage.getItem("id")}`).then(res=>{
      console.log(res.data);
      console.log(res.data.showData.movies);
      console.log(localStorage.getItem("id"));
      this.props.changeMaizuoTitle(res.data.cinemaData.nm);
     
      this.setState({
          showlist:res.data.cinemaData,
          movieslist:res.data.showData.movies,
         
      })
    })
    
 }
}
export default connect(
  null,

  {
    changeMaizuoTitle(data){

      return {
        type:"changeTitleForMaizuo",
        payload:data
      }
    }
  }
  )(Show)



